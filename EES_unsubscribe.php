...
<?php session_start();?>
	<?php
		...
		$sql = "DELETE FROM `subscribe` WHERE `subscribeAccountId` = '".$subscribeAccountId."' AND `subscribedAccountId` = '".$subscribedAccountId."'";
		if(mysql_query($sql)) {
			...
			if($result = mysql_query($sql)) {
				...
				//無法取得追蹤資訊
				$return_fail -> error = "1103";
				echo json_encode($return_fail);
			}
		}
		else {
			//DELETE失敗
			$return_fail -> error = "1102";
			echo json_encode($return_fail);
		}
	?>